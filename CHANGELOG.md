...
### Bug Fixes
- **A single-use DPoP proof leaked into the executor's default headers.** The nonce-retry path in `dpop_executor.py` did `self._default_headers.update(request["headers"])`, persisting that request's `DPoP` proof. A proof is single-use and bound to one `htm`/`htu` pair, so any later request built from those defaults carried a proof minted for the *previous* URL — which Okta rejects with `400 invalid_dpop_proof` / "'htu' claim in the DPoP proof JWT is invalid", the exact error the retry exists to recover from. Only the nonce is persisted now.
